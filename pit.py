...
def pick_word(category=None):
    ...
    try:
        ...
        # Fallback in case word bank is somehow empty
        print(f"[Error] Could not pick word: {e}")
        chosen_word     = "PYTHON"
        chosen_category = "Technology"
...
def update_word_grid():
    ...
    try:
        ...
        print(f"[Warning] Could not update word grid: {e}")
...
def check_game():
    ...
    try:
        ...
        print(f"[Error] check_game failed: {e}")
...
def disable_keyboard():
    try:
        ...
        print(f"[Warning] Could not disable keyboard: {e}")
...
def letter_click(letter):
    ...
    try:
        ...
        print(f"[Warning] letter_click error for '{letter}': {e}")
...
def on_key_press(event):
    try:
        ...
        print(f"[Warning] on_key_press error: {e}")

#restart game with option to keep same category
def restart_game(same_category=False):
    ...
    try:
        ...
        print(f"[Warning] restart_game UI error: {e}")
    except Exception as e:
        print(f"[Error] restart_game failed: {e}")
...
def show_main_menu():
    try:
        ...
        btn_frame.place(relx=0.5, y=470, anchor="center")
        ...
        tk.Label(root, text="← select a category or start randomly →",
                 font=("Courier", 10), fg="#878585", bg=BG).place(relx=0.5, y=570, anchor="center")

    except tk.TclError as e:
        print(f"[Error] show_main_menu failed: {e}")
...
def show_category_menu():
    try:
        ...
        print(f"[Error] show_category_menu failed: {e}")
...
# Top row: wrong guesses | hangman (center) | spacer
        ...
        hangman_canvas = tk.Canvas(top_row, width=230, height=230,
...
# Keyboard
        def patched_click(letter):
            try:
                ...
                print(f"[Warning] patched_click error: {e}")
...
# Bottom buttons
        ...
        print(f"[Error] show_game_screen failed: {e}")
    except Exception as e:
        print(f"[Error] Unexpected error in show_game_screen: {e}")
        ...
        for row in rows:
            ...
            for letter in row:
                btn = tk.Button(
                    ...
                    cursor="hand2",
                    command=lambda l=letter: click_fn(l),  # fixed: was click_fn(1)
                )
                btn.pack(side="left", padx=2)
                letter_buttons[letter] = btn
    except tk.TclError as e:
        print(f"[Error] create_keyboard_patched failed: {e}")
...
def use_hint(wrong_label_var):
    ...
    try:
        ...
        print(f"[Warning] use_hint error: {e}")
    except Exception as e:
        print(f"[Error] Unexpected error in use_hint: {e}")
